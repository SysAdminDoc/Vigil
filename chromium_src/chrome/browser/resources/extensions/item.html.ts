    ...
    <div id="content">
      ...
      <div>
        <div id="name-and-version" class="layout-horizontal-center">
          ...
          <!-- Vigil: Permission count badge -->
          ${this.data.permissions ? html`
            <span class="vigil-perm-badge"
                title="This extension has ${this.data.permissions.simplePermissions.length} permissions">
              ${this.data.permissions.simplePermissions.length} perms
            </span>` : ''}
        </div>
      </div>
      ...
      <!-- Vigil: Host access indicator -->
      ${this.data.permissions && this.data.permissions.runtimeHostPermissions ? html`
        <div class="vigil-host-access cr-secondary-text">
          ${this.data.permissions.runtimeHostPermissions.hasAllHosts ?
            html`<span class="vigil-risk-high">HOST ACCESS: ALL URLS</span>` :
            (this.data.permissions.runtimeHostPermissions.hosts.length > 0 ?
              html`<span class="vigil-risk-med">HOST ACCESS: ${this.data.permissions.runtimeHostPermissions.hosts.length} SITES</span>` :
              '')}
        </div>` : ''}
