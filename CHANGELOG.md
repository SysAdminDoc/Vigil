...
- **`devutils/smoke_test.py`** &mdash; CI-runnable smoke test that boots the built
  installer and verifies bundled uBO, `initial_preferences` applied, default search,
  and NTP wiring. Roadmap `N14`.

### Build & release pipeline (T4)

- **`.github/workflows/build-matrix.yml`** &mdash; candidate reusable-workflow + matrix
  rebuild of the 28-stage hand-chained CI, alongside the existing `main.yml`. Switch
  cutover happens after one green release. Roadmap `N7`.
- **`actions/attest-build-provenance`** step added to every artifact-emitting job
  (in the candidate workflow). Roadmap `N9`.
- **SignPath OSS submission scaffold** &mdash; workflow stubs the
  `signpath-org/github-action-submit-signing-request` invocation with a `TODO`
  pointing at the application steps in `docs/build-environment.md`. Roadmap `N8`.
- **winget manifest** at `dist/winget/SysAdminDoc.Vigil/<version>/` &mdash; ready to
  PR into `microsoft/winget-pkgs`. Roadmap `N15`.
...
- **`chromium_src/chrome/browser/resources/policy/policy.html`** &mdash; chrome://policy
  themed to match the rest of the Vigil overlays. Roadmap `X13`.
...
- **`tools/vigil-portable.cmd`** + portable sentinel logic doc &mdash; portable-mode
  launcher pattern (Supermium-style). Roadmap `X23`.
- **`ntp-extension/`** &mdash; the dark Vigil NTP repackaged as a bundled extension
  ...
  `chrome://newtab` wiring. Roadmap `N3`.
